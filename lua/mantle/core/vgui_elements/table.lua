...
function PANEL:Init()
	...
	self.vbar.grip:SetTooltipPanelOverride("MantleTooltip")
...
function PANEL:_createHeaderPanel()
	if not IsValid(self._topShadow) then
		self._topShadow = vgui.Create("Panel", self)
		self._topShadow:SetTooltipPanelOverride("MantleTooltip")
		...
		self._topShadow.Paint = function(_, w, h)
			...
			if Mantle.ui.convar.smooth and a > 0 then
				RNDX.Rect(0, 0, w, h):Blur():Fade(1, 0):Alpha(sa.a > 0 and a / sa.a or 0):Draw()
			end
		end
	end

	if not IsValid(self.header) then
	...
	if not IsValid(self.headerText) then
		self.headerText = vgui.Create("Panel", self)
		self.headerText:SetTooltipPanelOverride("MantleTooltip")
...
function PANEL:_isInternal(child)
	return child == self.content
		or child == self.header
		or child == self.headerText
		or child == self._topShadow
		or child == self.vbar
		or child == self.vbar.grip
end
...
function PANEL:SortByColumn(columnIndex)
	...
	if not column or not column.sortable then
		return
	end
...
function PANEL:OnMousePressed(mc)
	...
	if
		IsValid(hovered)
		and (
			hovered == self.headerText
			or Mantle.util.isDescendantOf(hovered, self.headerText)
			or hovered == self.vbar
			or Mantle.util.isDescendantOf(hovered, self.vbar)
		)
	then
		return
	end
...
function PANEL:CreateHeader()
	...
	for i, column in ipairs(self.columns) do
		...
		btn:SetTooltipPanelOverride("MantleTooltip")
		btn:SetText("")
...
function PANEL:CreateRow(rowIndex, rowData)
	...
	row:SetTooltipPanelOverride("MantleTooltip")
	row:Dock(TOP)
	...
	for i, column in ipairs(self.columns) do
		...
		cell:SetTooltipPanelOverride("MantleTooltip")
...
function PANEL:RebuildRows()
	...
	self._hoverBar:SetTooltipPanelOverride("MantleTooltip")
	...
	self._hoverBar.Think = function()
		...
		if IsValid(hovered) then
			...
			if math.abs(self._hoverX - hovered:GetX()) < 0.5 then
				self._hoverX = hovered:GetX()
			end
			if math.abs(self._hoverY - hovered:GetY()) < 0.5 then
				self._hoverY = hovered:GetY()
			end
			if math.abs(self._hoverW - hovered:GetWide()) < 0.5 then
				self._hoverW = hovered:GetWide()
			end
			if math.abs(self._hoverH - hovered:GetTall()) < 0.5 then
				self._hoverH = hovered:GetTall()
			end
		else

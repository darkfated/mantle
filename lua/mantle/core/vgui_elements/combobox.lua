...
function PANEL:Init()
	...
	self.btn:SetTooltipPanelOverride("MantleTooltip")
	self.btn:Dock(FILL)
	self.btn:SetText("")
	...
	self.btn.Paint = function(button, w, h)
		...
		if self.hoverAnim > 0.01 then
			...
			RNDX.Rect(0, 0, w, h):Rad(RADIUS):Color(Color(hv.r, hv.g, hv.b, math_floor(hv.a * self.hoverAnim))):Draw()
		end
...
vgui.Register("MantleComboBox", PANEL)

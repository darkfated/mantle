...
function PANEL:Init()
	...
	self._hoverBar.Paint = function(_, w, h)
		...
		RNDX.Rect(0, 0, w, h):Rad(8):Color(Color(hv.r, hv.g, hv.b, math_floor(hv.a * a))):Draw()
	end
	...
	self.Think = function()
		...
		if
			not IsValid(hovered)
			and IsValid(self._hoverParent)
			and self._hoverParent._submenu == self
			and self._hoverParent._submenu_open
		then
			for _, item in ipairs(self.Items) do
				if IsValid(item) and item.Text ~= nil then
					hovered = item
...
function PANEL:Paint(w, h)
	...
	if not self._disableBlur then
		RNDX.Rect(0, 0, w, h):Rad(RADIUS):KBlur(4, 2):Draw()
	end

	local bg = Mantle.color.background_panelpopup
	RNDX.Rect(0, 0, w, h):Rad(RADIUS):Color(Color(bg.r, bg.g, bg.b, math_floor((bg.a or 150) * aMul))):Draw()

	RNDX.Rect(0, 0, w, h):Rad(RADIUS):Color(Mantle.color.window_shadow):Outline(1):Draw()
end
...
function PANEL:AddOption(text, func, icon, optData)
	...
	option:SetTooltipPanelOverride("MantleTooltip")
	option:SetText("")
	option:Dock(TOP)
	...
	option.Paint = function(pnl, w, h)
		if pnl._submenu and not pnl._submenu_open and pnl:IsHovered() then
			pnl:OpenSubMenu()
		end
		...
		if pnl._iconMat then
			RNDX.Rect(12, (h - iconSize) / 2, iconSize, iconSize):Material(pnl._iconMat):Color(Mantle.color.text):Draw()
			textX = 36
		end
	...
	function option:AddSubMenu()
		if IsValid(option._submenu) then
			option._submenu:Remove()
		end
		...
		function option:OpenSubMenu()
			...
			for _, sibling in ipairs(self:GetParent().Items or {}) do
				if sibling ~= self and sibling.CloseSubMenu then
		...
		local function isAnySubmenuHovered(opt)
			...
			if opt._submenu and IsValid(opt._submenu) and opt._submenu:IsVisible() then
				if isAnySubmenuHovered(opt._submenu) then
					return true
				end
				for _, item in ipairs(opt._submenu.Items or {}) do
					if isAnySubmenuHovered(item) then
						return true
					end
				end
			end
		...
		option.OnCursorExited = function(pnl)
			timer.Simple(0.15, function()
				if not isAnySubmenuHovered(pnl) and IsValid(pnl) then
					pnl:CloseSubMenu()
				end
			end)
		end
		submenu.OnCursorExited = function(pnl)
			timer.Simple(0.15, function()
				if not isAnySubmenuHovered(option) and IsValid(pnl) then
					option:CloseSubMenu()
				end
			end)
		end
...
function PANEL:AddSpacer()
	...
	spacer:SetTooltipPanelOverride("MantleTooltip")
	spacer:Dock(TOP)
	spacer:DockMargin(6, 4, 6, 4)
	spacer:SetTall(1)
	spacer.sumTall = 9
	spacer.Paint = function(_, w, h)
		RNDX.Rect(0, 0, w, h):Color(Mantle.color.window_shadow):Draw()
	end
...
vgui.Register("MantleDermaMenu", PANEL)

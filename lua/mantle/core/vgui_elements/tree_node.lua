...
AccessorFunc(PANEL, "m_strFolder", "Folder", FORCE_STRING)
AccessorFunc(PANEL, "m_strFileName", "FileName", FORCE_STRING)
AccessorFunc(PANEL, "m_strPathID", "PathID", FORCE_STRING)
AccessorFunc(PANEL, "m_strWildCard", "WildCard", FORCE_STRING)
AccessorFunc(PANEL, "m_bNeedsPopulating", "NeedsPopulating", FORCE_BOOL)
...
AccessorFunc(PANEL, "m_strDraggableName", "DraggableName", FORCE_STRING)

surface.SetFont("Fated.13")

function PANEL:Init()
	self:SetDoubleClickToOpen(true)

	self.Label = vgui.Create("DLabel", self)
	self.Label:SetTooltipPanelOverride("MantleTooltip")
	self.Label:SetTextInset(32, 0)
	self.Label:SetContentAlignment(4)
	self.Label:SetSelectable(true)
	self.Label:SetMouseInputEnabled(true)
	self.Label:SetCursor("arrow")
	self.Label:SetFont(Mantle.func.cache_font(13))
	self.Label:SetTextColor(Mantle.color.text)
	...
	self.Label.DoRightClick = function()
		self:InternalDoRightClick()
	end
	self.Label.DragHover = function(_, t)
		self:DragHover(t)
	end
	self.Label.Paint = function(s, w, h)
		if s:IsSelected() then
			RNDX.Draw(16, 0, 0, w, h, Mantle.color.button_hovered, RNDX.SHAPE_IOS)
		end
		if s:IsHovered() then
	...
	self.Expander:SetTooltipPanelOverride("MantleTooltip")
	self.Expander.DoClick = function()
		self:SetExpanded(not self:GetExpanded())
	end
	self.Expander:SetVisible(false)

	self.Icon = vgui.Create("DImage", self)
	self.Icon:SetTooltipPanelOverride("MantleTooltip")
...
function PANEL:InternalDoClick()
	...
	if self:GetRoot():DoClick(self) then
		return
	end
...
function PANEL:OnNodeAdded(node) end

function PANEL:InternalDoRightClick()
	if self:DoRightClick() then
		return
	end
	if self:GetRoot():DoRightClick(self) then
		return
	end
end
...
function PANEL:AnimSlide(anim, delta, data)
	if not IsValid(self.ChildNodes) then
		return anim:Stop()
	end
...
function PANEL:ExpandRecurse(bExpand)
	...
	for _, Child in pairs(self.ChildNodes:GetChildren()) do
...
function PANEL:AddNode(strName, strIcon)
	...
	local pNode = vgui.Create(self.ClassName, self)
...
function PANEL:FilePopulateCallback(files, folders, foldername, path, bAndChildren, wildcard)
	...
	local FileCount = 0
	if folders then
		for _, File in SortedPairsByValue(folders) do
	...
	if showfiles then
		for _, File in SortedPairs(files) do
			local Node = self:AddNode(File, "icon16/page_white.png")
...
function PANEL:FilePopulate(bAndChildren, bExpand)
	if not self:GetNeedsPopulating() then
		return
	end
	...
	if not folder or not wildcard or not path then
		return false
	end
...
function PANEL:PopulateChildrenAndSelf(bExpand)
	if self:FilePopulate(true, bExpand) then
		return true
	end
...
function PANEL:Copy()
	local copy = vgui.Create(self.ClassName, self:GetParent())
	...
	if self.ChildNodes then
		for _, v in ipairs(self.ChildNodes:GetChildren()) do
...
function PANEL:SetupCopy(copy) end

vgui.Register("MantleTree_Node", PANEL)

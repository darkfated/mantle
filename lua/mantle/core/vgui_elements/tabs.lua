...
function PANEL:Init()
	...
	self._tabShadow:SetTooltipPanelOverride("MantleTooltip")
...
function PANEL:_createTabButton(tab, id)
	...
	btn.Paint = function(s, w, h)
		...
		if self.tabStyle == "modern" then
			...
			if tab.icon then
				RNDX.Rect(padding, (h - 16) * 0.5, 16, 16):Material(tab.icon):Color(colorIcon):Draw()
			end
			...
			if tab.icon then
				RNDX.Rect(9, 9, 16, 16):Material(tab.icon):Color(colorIcon):Draw()
			else
				RNDX.Rect(9, 9, 16, 16):Rad(24):Color(colorIcon):Draw()
			end
		end
	end
...
function PANEL:Think()
	...
	if not self._indicator_inited then
...
function PANEL:SetTabStyle(style)
	if style ~= "modern" and style ~= "classic" then
		return
	end
	if self.tabStyle == style then
		return
	end
...
vgui.Register("MantleTabs", PANEL)

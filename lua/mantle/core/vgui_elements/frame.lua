...
function PANEL:Init()
	...
	function self.header:OnMousePressed(key)
		...
		if key ~= MOUSE_LEFT or not frame.draggable then
			return
		end
	...
	self.cls.Paint = function(btn, w, h)
		if btn:IsHovered() then
			RNDX.Rect(2, 2, w - 4, h - 4):Rad(6):Color(Mantle.color.hover_overlay):Draw()
		end

		RNDX.Rect(4, 4, w - 8, h - 8):Color(Mantle.color.header_text):Material(mat_close):Draw()
	end
	...
	self.cls.DoRightClick = function()
		...
		DM:AddOption(Mantle.lang.get("mantle", "frame_move_from_menu"), function()
			self:SetKeyBoardInputEnabled(not boolInput)
		end, not boolInput and "icon16/bullet_green.png" or "icon16/bullet_red.png")
...
function PANEL:Notify(text, duration, col)
	...
	mp.Paint = function(_, w, h)
		RNDX.Rect(0, 0, w, h):Rad(14):Blur(2):Draw()
		RNDX.Rect(0, 0, w, h):Rad(14):Color(Mantle.color.background_panelpopup):Draw()
		RNDX.Rect(0, 0, w, h):Rad(14):Color(Mantle.color.notify_outline):Outline(1):Draw()
...
function PANEL:Paint(w, h)
	...
	RNDX.Rect(0, 0, w, h):Radii(6, 6, 6, 6):Color(col):Shadow(20, 12):Draw()
	RNDX.Rect(0, 0, w, h):Radii(6, 6, 6, 6):Color(col):Shadow(6, 3):Draw()
	...
	if alphaBg then
		RNDX.Rect(0, 0, w, h):Radii(bodyRad, bodyRad, 6, 6):KBlur(4, 2):Draw()
	end
	...
	if not lite then
		RNDX.Rect(0, 0, w, headerTall):Radii(6, 6, 0, 0):Color(Mantle.color.header):Draw()
	end
